...
from .types import Matrix, NextHop, WeightedGraph


def floyd_warshall(graph: WeightedGraph) -> Tuple[Matrix, NextHop]:
...
def reconstruct_fw_path(next_hop: NextHop, source: Any, target: Any) -> List[Any]:
    """Reconstruct path between source and target from next-hop table."""
    if source not in next_hop or target not in next_hop[source]:
        return []
    if source != target and next_hop[source][target] is None:
        return []
